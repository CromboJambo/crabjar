...
The crabjar system follows the rule that "knowing what happened does not grant the right to change what happens." This principle is enforced across all agent components.

### Agent (crabjar) Characteristics:
...
1. **Configuration changes**: Only users can modify .crabjar_config.toml files; agent cannot initiate config updates  
...
- During configuration changes, user must explicitly modify .crabjar_config.toml while agent observes and reports state before/after  
