...
name: rust-nightly
description: |
  Install and manage Rust nightly toolchain via rustup. Use whenever the user
  mentions installing Rust nightly, switching to nightly, rustup nightly,
  rust-toolchain.toml nightly, or needs nightly-only features (unstable
  lints, new lang items, rustdoc JSON). Also trigger when the user says
  "install rust", "setup rust", "rust toolchain", "rustup install", or
  "nightly toolchain". Does NOT trigger for stable-only workflows or when
  a stable toolchain is already configured.
---
