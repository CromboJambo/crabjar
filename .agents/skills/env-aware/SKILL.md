...
The agent should always check `${PROJECT}/environment_manifest.json` before proposing large-scale operations. This file contains the ground truth for:
...
When testing `${PROJECT}` commands or tool calls, use simulated environments. If `${PROJECT}/testing/configs/` exists, use its mock configs. If not, create tier configs (low_ram, gpu_only, btrfs) before testing.

## Workflow Instructions

### When asked about system identity ("whoami", "whatami"):
1. **Read the Manifest**: Read `${PROJECT}/environment_manifest.json` if it exists.
...
6. **Update Manifest**: Write the live probe results back to `${PROJECT}/environment_manifest.json` so it stays current.
...
1. **Locate Mocks**: Look into `${PROJECT}/testing/configs/` to find a relevant mock configuration.
...
## Reference Files
* `${PROJECT}/environment_manifest.json`: The source of truth for the current environment. (May be sparse — use live probes as fallback.)
