...
async fn run_tui(
    ...
    while running {
        terminal.draw(|frame| {
            ...
            // Agent loop status
            let agent_block = Block::default().borders(Borders::ALL).title(" Agent Loop ");
            let agent_text = vec![
            ...
            let plugin_rows: Vec<Vec<Line>> = plugins
                .iter()
                .map(|p| {
                    vec![
            ...
            let plugin_block = Block::default().borders(Borders::ALL).title(" Plugins ");
