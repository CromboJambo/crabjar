...
impl ZellijBackend {
    ...
    async fn run_zellij(&self, args: &[&str]) -> anyhow::Result<String> {
        ...
        let output = cmd
            .output()
            .await
            .with_context(|| format!("Failed to execute zellij {:?}", args))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr).to_string();
            anyhow::bail!(
                "zellij command failed (exit {:?}): {}",
                output.status,
                stderr
            );
        }
    ...
    async fn run_action(&self, args: &[&str]) -> anyhow::Result<String> {
        ...
        let output = cmd
            .output()
            .await
            .with_context(|| format!("Failed to execute zellij action {:?}", args))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr).to_string();
            anyhow::bail!(
                "zellij action failed (exit {:?}): {}",
                output.status,
                stderr
            );
        }
...
impl TerminalBackend for ZellijBackend {
    ...
    async fn spawn(&self, session_name: &str, working_dir: &Path) -> anyhow::Result<SpawnResult> {
        ...
        let output = cmd
            .output()
            .await
            .with_context(|| "Failed to start zellij server")?;
    ...
    async fn read_output(&self, _session_name: &str, lines: usize) -> anyhow::Result<String> {
