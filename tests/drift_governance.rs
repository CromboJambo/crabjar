...
async fn drift_governance_loop() {
    ...
    let querier = agent_context::state_docs::StateDocQuerier::new(conn.clone(), docs_dir.clone());
    ...
    agent_context::state_docs::migrate(&conn2).unwrap();
    agent_context::state_docs::indexer::index_all_docs(&conn2, &docs_dir).unwrap();
    
    // Verify drift is resolved
    let querier2 = agent_context::state_docs::StateDocQuerier::new(conn2.clone(), docs_dir.clone());
